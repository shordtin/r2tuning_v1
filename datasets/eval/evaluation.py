...
from .utils import (compute_average_precision_detection, compute_temporal_iou_batch_cross,
...
def compute_mr_r5(submission, ground_truth, iou_thds=np.linspace(0.3, 0.95, 14)):
    ...
    iou_thds = [float(f"{e:.2f}") for e in iou_thds]
    pred_qid2window = {
        d["qid"]: [x[:2] for x in d["pred_relevant_windows"][:5]]
        for d in submission
    }
    gt_qid2window = {}
...
def mk_gt_scores(gt_data, clip_length=1):
    ...
    saliency_scores_relevant_clips = np.array(
        gt_data["saliency_scores"])  # (#relevant_clip_ids, 3)
...
def print_submission(submission):
    ...
    for i, s in enumerate(submission):
        print(f"qid: {s['qid']}, vid: {s['vid']}")
        print(f"pred_relevant_windows: {s['pred_relevant_windows'][:5]}")
        print("-" * 50)
        if i >= num:
            break


def vtg_eval(submission, ground_truth, verbose=True, match_number=True):
    """
    Args:
    ...
    if "pred_relevant_windows" in submission[0]:
        ...
        # print(moment_ret_scores)
        eval_metrics.update(moment_ret_scores)
        moment_ret_scores_brief = {
            "MR-full-mAP": moment_ret_scores["full"]["MR-mAP"]["average"],
            "MR-full-mAP@0.5": moment_ret_scores["full"]["MR-mAP"]["0.5"],
            "MR-full-mAP@0.55": moment_ret_scores["full"]["MR-mAP"]["0.55"],
            "MR-full-mAP@0.6": moment_ret_scores["full"]["MR-mAP"]["0.6"],
            "MR-full-mAP@0.65": moment_ret_scores["full"]["MR-mAP"]["0.65"],
            "MR-full-mAP@0.7": moment_ret_scores["full"]["MR-mAP"]["0.7"],
            "MR-full-mAP@0.75": moment_ret_scores["full"]["MR-mAP"]["0.75"],
            "MR-full-mAP@0.8": moment_ret_scores["full"]["MR-mAP"]["0.8"],
            "MR-full-mAP@0.85": moment_ret_scores["full"]["MR-mAP"]["0.85"],
            "MR-full-mAP@0.9": moment_ret_scores["full"]["MR-mAP"]["0.9"],
            "MR-full-mAP@0.95": moment_ret_scores["full"]["MR-mAP"]["0.95"],
            ...
            "MR-full-R1@0.75": moment_ret_scores["full"]["MR-R1"]["0.75"],
            "MR-full-R1@0.8": moment_ret_scores["full"]["MR-R1"]["0.8"],
            "MR-full-R1@0.85": moment_ret_scores["full"]["MR-R1"]["0.85"],
            "MR-full-R1@0.9": moment_ret_scores["full"]["MR-R1"]["0.9"],
            "MR-full-R1@0.95": moment_ret_scores["full"]["MR-R1"]["0.95"],
            ...
            # "MR-full-R5@0.75": moment_ret_scores["full"]["MR-R5"]["0.75"],
            # "MR-full-R5@0.8": moment_ret_scores["full"]["MR-R5"]["0.8"],
            # "MR-full-R5@0.85": moment_ret_scores["full"]["MR-R5"]["0.85"],
            # "MR-full-R5@0.9": moment_ret_scores["full"]["MR-R5"]["0.9"],
            # "MR-full-R5@0.95": moment_ret_scores["full"]["MR-R5"]["0.95"],
    ...
    if ("pred_saliency_scores" in submission[0]) and ("saliency_scores"
                                                      in ground_truth[0]):
        if isinstance(ground_truth[0]['saliency_scores'],
                      list):  # some item might be nan not list.
            ...
            eval_metrics_brief["HL-min-VeryGood-mAP"] = eval_metrics_brief.pop(
                "HL-min-VeryGood-mAP")
            eval_metrics_brief["HL-min-VeryGood-Hit1"] = eval_metrics_brief.pop(
...
def eval_main():
    ...
    results = vtg_eval(submission, gt, verbose=verbose)
    if verbose:
